...
En este equipo la ruta con NVIDIA no activa el monitor, mientras AMD integrada sí lo hace. El driver prefiere ahora una GPU física de bajo consumo mediante DXGI/D3DKMT, excluyendo adaptadores virtuales y de software antes de llamar a `IddCxAdapterSetRenderAdapter`. No fija una marca ni un LUID entre reinicios. La constante `RenderPreference` de `RenderAdapter.h` permite ajustar la preferencia al compilar; no se ha validado esta elección en otros equipos. El worker informa también de las estadísticas de los frames que descarta.
...
Actualización anterior del 7 de septiembre: instalado y seleccionado `oem100.inf` **0.1.0.8**, que añade el [prototipo de frames directos](../../docs/adr/0011-driver-frame-transfer-probe.md). Mantiene la ruta anterior cuando no se solicita el mapping de prueba. Se conserva el paquete firmado 0.1.0.7 en `target/windows-idd-test-v0.1.0.7/` y en Driver Store (`oem99.inf`), además del respaldo original 0.1.0.0. Las pruebas de activación y captura anteriores vuelven a pasar.

Actualización más reciente: **0.1.0.9, `oem101.inf`**, con texturas D3D11 compartidas. El [ADR 0012](../../docs/adr/0012-shared-d3d11-frame-probe.md) recoge la comparación con CPU y sus límites. Se conserva 0.1.0.8 firmado en `target/windows-idd-test-v0.1.0.8/`. Para repetir la aceptación GPU, iniciar `windowdeck-display --gpu-frame-broker` como administrador y ejecutar `powershell -NoProfile -ExecutionPolicy Bypass -File driver/windows-idd/test-driver-frames.ps1 -Gpu` desde una terminal normal, sin monitor activo. Sin `-Gpu`, el script prueba CPU con `--frame-broker`; ambos exigen 0.1.0.9 por defecto. Los modos `--gpu-frame-source` y `--frame-source` son auxiliares binarios para Rust, no comandos interactivos.
...
## Streaming CPU experimental

`windowdeck-display --cpu-frame-stream` usa el mismo `--frame-broker` del probe CPU, pero emite el escritorio real como BGRA continuo de 1280 × 800, sin cabecera ni patrón. Lo inicia el host con `--driver-h264`; no debe ejecutarse interactivamente porque stdin es el pipe de vida del host y stdout contiene datos binarios. Repite el último frame a una cadencia nominal de 60 FPS y registra por separado salidas y actualizaciones del driver. Véase [ADR 0013](../../docs/adr/0013-driver-cpu-h264.md). Solo cambia el auxiliar: no requiere reinstalar el driver.
