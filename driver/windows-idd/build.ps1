...
$iddVs = & $iddVswhere -latest -products '*' -version '[18.0,19.0)' -requires Component.Microsoft.Windows.DriverKit.BuildTools -property installationPath
...
foreach ($iddProject in @('Control.vcxproj', 'WindowDeckDisplay.vcxproj')) {
    & $iddMsbuild (Join-Path $PSScriptRoot $iddProject) /p:Configuration=Release /p:Platform=x64 /p:PreferredToolArchitecture=x64 /v:minimal /nologo
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LICENSE') -Destination (Join-Path $iddOutput 'WindowDeckDisplay')
