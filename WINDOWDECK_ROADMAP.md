...
Estado actual: host y cliente negocian y reproducen una emisión H.264 continua mediante MPEG-TS, el protocolo v3 y FFplay; la prueba actual está configurada a 1280 × 800, 60 FPS y 4 Mbps. El prototipo usa `ddagrab` y `libx264` sin colas de aplicación, registra métricas cada segundo, conserva una [línea base local](docs/testing.md) y genera un Flatpak provisional para probar el cliente en una Steam Deck real. Aún falta validar visualmente los 60 FPS y sustituir el encoder software por aceleración hardware.
